...
// 1. Remove the global fail, drift, errata block
const beforeStr = 'const seasonWeather = getSeasonallyAccurateWeather(date, authorLocation);\n';
const afterStr = '\n    const cleanNames = selectedItems.map(i => i.title';
const startIdx = content.indexOf(beforeStr) + beforeStr.length;
const endIdx = content.indexOf(afterStr);

if (startIdx !== -1 && endIdx !== -1) {
    content = content.substring(0, startIdx) + content.substring(endIdx);
} else {
    console.error("Could not find the global fail/drift block");
    process.exit(1);
}

// 2. Add local declarations before switch
