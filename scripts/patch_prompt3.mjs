...
// 1. Erase old variables using regex (whitespace agnostic)
content = content.replace(/const\s+fail\s*=\s*getRandomItem[\s\S]*?\};\s*/, '');
content = content.replace(/const\s+drift\s*=\s*getRandomItem[\s\S]*?\};\s*/, '');
content = content.replace(/const\s+errata\s*=\s*\(Math\.random[\s\S]*?:\s*null;\s*/, '');
...
const ruleRegex = /7 NUCLEAR ANTI-AI RULES \(MANDATORY\):[\s\S]*?7\. 4-STEP BLUEPRINT: Deliver[^\n]*/;
...
content = content.replace(ruleRegex, ruleReplace);
